...
refst=align.refst;%1
 ...
 elseif shiftT1bool==true;
     ...
     for k=1:1
         k
         xx0=x(refst,(ts11-range)*sr:(ts11+win+range)*sr);%reference seismograms
         xx0(1:range*sr)=0;
         xx0(end-range*sr:end)=0;
         xx02=sum(xx0.^2);
...
%              ['k= ' num2str(k) ' i= ' mum2str(i)]
             ...
             xx=x(i,(ts11-range)*sr:(ts11+win+range)*sr);
...
%     shifting the data with y2;
    ...
    for i=1:nel
          x1(i,:)=specshift(x0(i,:),timeshift1(i)*sr);
    end
    %%%%%%%%%%%%%%%%%%%%%
    if isfield(ret,'xori')
        for i=1:nel
            ret.xori(i,:)=specshift(ret.xori(i,:),timeshift1(i)*sr);
