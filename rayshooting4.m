...
%I0S = [19.5:0.02:19.6];
I0S = 8:0.01:13;         % initial angles, in degrees
...
depth1=[
   ...
   12.8450
    ...
    6.2000
    9.6000];
toa=zeros(1,length(depth1));
depf=zeros(1,length(depth1));
0.005;
...
for k=1:length(I0S),
 ...
 % Or we could use ode45
  ...
  deptest=interp1(xray/1e3,zray/1e3,dist1);
  for j=1:length(depth1)
       if abs(deptest(j)-depth1(j))<0.02
            tem(j)=interp1(xray/1e3,zray/1e3,dist1(j)-0.01);
             toa(j)=atan((tem(j)-deptest(j))/0.01)*180/pi;
             depf(j)=deptest(j);
             ioa(j)=I0S(k);
       end
  end
...
% end
% 
%         [C,H]=contourf(lat/1e3, lon/1e3, MM3,1/10*[0 500 1000 1500 2000 2500 3000 3500 4000 4500 5000 5500 6000 6500 7000 7500 8000 8500 9000 9500 10000 10500 11000 11500 12000  ]);%[50 100 300 400 500 600 700 800 900 1000 1200 1500 2000 2500 3000 ]
