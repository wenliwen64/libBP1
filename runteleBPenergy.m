function runteleBPenergy(DataFile)
load(DataFile);
addpath ./libBP;
load ptimes;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
y0=zeros(n,2*win*sr);
            ...
            for p=1:ps
                ...
                for q=1:qs
                    ...
                    for k=1:n
                           y(k,:)=x0(k,floor((tl+sd(k))*sr):floor((tl+win+sd(k))*sr-1)).^2;
%                         y0(k,:)=specshift(x0(k,(tl-1/2*win)*sr:(tl+3/2*win)*sr-1),sd(k)*sr);
%                         y(k,:)=y0(k,1/2*win*sr:3/2*win*sr-1).^2;%/std(y(k,tl*sr:tl*sr+windowLength*sr-1));
                    end
                    Pm(p,q)=sum(sum(y(:,:),1));
