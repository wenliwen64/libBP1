...
for ii = 1:nsta
    ii
    [dummy, timeshift0] = retracing_liwen2([ret.lat(ii), ret.lon(ii)], [ep_lat, ep_lon], evt_depth, epicenter, bparea_span);
    count = 1;
    for i = 1:yslices % latitude
        for j = 1:xslices % longitude
            ...
            [loc_new_grid_flat, loc_new_grid, traveltime_flat, traveltime_actual] = retracing_liwen2(loc_sta, loc_grid, evt_depth, epicenter, bparea_span);
            loc_new_grid_x(count) = loc_new_grid(2);
            loc_new_grid_y(count) = loc_new_grid(1);
            loc_flat_grid_x(count) = loc_new_grid_flat(2);
            loc_flat_grid_y(count) = loc_new_grid_flat(1);
            ...
            timeshift(count) = traveltime_actual; % -timeshift0;
            %timeshift_temp;
            timeshift_old(count) = traveltime_flat;
    ...
    ret1.timeshift(:, :, ii) = v1(:, :) - ep_newtimeshift; %- ep_newtimeshift; % v1(xslices/2+1, yslices/2+1); % new_grid_timeshift(ii, :, :);
