...
% %%%=============================%%%
 ...
 for i = 1:N
      ...
      if STA/LTA > 2.7
         temp_lat = logfile(LOCS(i), 2);
         temp_lon = logfile(LOCS(i), 3);
         if((temp_lat-lat0) > 1 || (temp_lat-lat0) < -2) continue; end
         if((temp_lon-lon0) > 2 || (temp_lon-lon0) < -1) continue; end
         PKS0(kt)=PKS(i);
         LOCS0(kt)=LOCS(i);
         kt = kt + 1;
      end
...
% %%%===================================================================%%%
% 
% %============Cut the buffer area earthquakes===========================
% 
...
M_bp_usgs = horzcat(shift_bp_usgs, az_bp_usgs, log10(logfile(LOCS0(ind_bp), 4))/max(log10(logfile(LOCS0(ind_bp), 4))),...
                    eqinfo(I(ind_usgs), 10), logfile(LOCS0(ind_bp), 3), ...
                    logfile(LOCS0(ind_bp), 2)/max(logfile(LOCS0(ind_bp), 2)) , eqinfo(I(ind_usgs), 9)-dlon, ...
                    eqinfo(I(ind_usgs), 8)-dlat);
...
%figure(11);
%scatter(Mag(LOCS0(ind_bp)), eqinfo(I(ind_usgs), 10));
