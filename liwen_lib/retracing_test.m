...
for ii = 1:nsta
    ...
    for i = 1:40 % latitude
        for j = 1:40 % longitude
            loc_grid = [bparea_span(1,1)+i*lat_step+ep_lat, bparea_span(2,1)+j*lon_step+ep_lon];
            ...
            loc_old_grid_x(count) = bparea_span(2,1) + j*lon_step + ep_lon;
            loc_old_grid_y(count) = bparea_span(1,1) + i*lat_step + ep_lat;
