function loc_new_grid = retracing_liwen(loc_sta, loc_grid, evt_depth)
...
%      asind(sin_takeoff_angle)
     ...
     ini_point = [loc_grid(1), loc_grid(2), -evt_depth];
     v0_point = [0, 0, -40];  % moho depth
     ...
     [loc_third_point, downward_vec] = raytracing_liwen_downward(ini_point, ini_vec, v0_point, ...
         moho_norm_flat_vec, third_layer_depth, nindex_down);
     [loc_new_grid, upward_vec] = raytracing_liwen_downward(loc_third_point, -downward_vec, ...
         v0_point, moho_norm_actual_vec, evt_depth, nindex_up);
