...
for j=1:nel
    xorin(j,:)=coe(j)*x1(j,round((ta1+sd(j))*sr):round((ta1+sd(j))*sr)+L*sr-1);
end
for j=1:nel
    xr(j,:)=coe(j)*x1(j,:);
end
...
D = fdesign.highpass('Fst,Fp,Ast,Ap',70,80,200,1,200);
    butterbps = design(D,'equiripple');
figure(27);
%[B,A]= butter(2, [12.5 14.5]/sr); %construct the filter for 8 to 12 hz
[B,A]= butter(5, 140/sr,'high'); %construct the filter for 8 to 12 hz
...
y=zeros(nel,L*sr);
for i=1:nel
    y(i,:)= filter(butterbps,xorin(i,:));
        xorin(i,:)= filter(butterbps,xorin(i,:));
end
figure(14);
for i=1:nel
    subplot(nel,1,i);
...
% end
 s1=linspace(0,sr,0.5*sr);
          [s c ph ci phi]=cmtm1(xorin(1,tl*sr:th*sr-1),xorin(2,tl*sr:th*sr-1),0.005,1.5,0,0,0);
...
for i=1:nel
    for j=1:nel
          [c ph ci ]=cmtm2(xorin(i,tl*sr:th*sr-1),xorin(j,tl*sr:th*sr-1),2);
          for k=1:length(s)
              S(k,i,j)=c(k);
          end
    end
