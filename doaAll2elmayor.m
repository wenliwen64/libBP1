function ret=doaAll2elmayor(ret,opr)
...
%                       [c ph ci ]=cmtm2normalized(xi,xj,Nw);
% %                     for k=1:length(s)
% %                         if isnan(c0(k))
% %                             c0(k)=1;
% %                         end
...
%                         Pw1(p,q)=(wi*(a'*Us*a)/(a'*a));
                ...
                %pause
                                ...
                                maxp=real(max(max(Pm1)));
%                      Pm1=(Pm1-minp)/(maxp-minp);% normalization
%                      Pm1=Pm1/real(max(max(Pm1)));%old normalization
                      maxp=real(max(max(Pm1)));
                      ['maxp=' num2str(maxp)]
                Pm=Pm+Pm1;
%                 Pw=Pw+Pw1;
