function [uxx uyy val distpeak max1]=tprfslowcali(ux,uy,Pm,bux,buy,lat0,lon0,latm,lonm,dist,faultorientation) 
...
az=zeros(1,n);
val=zeros(1,n);
slowpeak=min([ sqrt(bux^2+buy^2) 0.6]);
...
obaz=az(i)-180;
tbaz=obaz+22.65*cosd(obaz+50.4);% fitting by all the fletcher points
% tbaz=obaz-13.65*cosd(obaz+246.58);%fitting by 6 aftershocks from mine
az(i)=tbaz+180;

    uxx(i)=slowpeak*cosd(az(i));
    uyy(i)=slowpeak*sind(az(i));
