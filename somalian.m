...
%     end
    ...
    disB=lonB*0;
    for kk=2:length(B)
        disB(kk)=disB(kk-1)+((lonB(kk)-lonB(kk-1))^2+(latB(kk)-latB(kk-1))^2)^0.5*110;
    end
...
title(name(ii,:));
subplot(4,1,3);
x1=real(hilbert(x)*exp(-1i*180/180*pi));
plot(1:length1,x1/5,'b-',1:length1,0,'g')

subplot(4,1,4);
plot(disA,val*5,'b',disB,bath,'r');
