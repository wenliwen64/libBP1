...
B=[1:8 11 13 15:16]%1:16%[1:8 10:13 15 16];
...
%     %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  ret=struct('r',r,...%the position of the stations
    ...
    't1',(1:length(rdis))*0,...% theoretical arrival times
    'x',x0,...% original data 
    'x1',x0,...
    'x2',x2,...% aligned unfiltered
    'sr',sr,...% sample rate
    'ori',105,...% origin time
...
alignopr.range=50;
load ven61timeshift;
alignopr.timeShiftKnown=timeshift;
ret=seismoAlign(ret,alignopr);

for i=1:nel
    ret.x2(i,:)=specshift(ret.x2(i,:),ret.timeshift(i)*sr);
end
...
doaopr.slidingWindowViewRange=[-20 20 16];
...
doaopr.beamOrientation=115;
doaopr.saveProj=NN;
doaopr.Mm=2;
doaopr.saveDir='/home/lsmeng/matlab/haitiUS/gf/save_half6/';
