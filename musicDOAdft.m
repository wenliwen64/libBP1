function [X,Uv,A]=musicDOA(yy,time,tts1)
M=1;%number of signals
%k=@(f0,ux,uy) 2*pi*f0*[ux uy (1/0.7^2-ux^2-uy^2)^0.5];% wave vector
k=@(f0,ux,uy) 2*pi*f0*[ux uy (1/0.7^2-ux^2-uy^2)^0.5];% wave vector
...
%     Rxx=Rxx+X(:,p)*X(:,p)';
% end
w=linspace(0,samplerate,samplerate*time);
fh=30;
fl=20;
ifh=round(interp1(w,1:length(X(1,:)),fh));
ifl=round(interp1(w,1:length(X(1,:)),fl));
ps=100;
qs=100;
Xm=zeros(ps,qs);
Ym=zeros(ps,qs);
...
for i=ifl:ifh
    ...
    Un=un*un';
    wi=round(interp1(1:length(X(1,:)),w,i));
    for p=1:ps
        for q=1:qs
            
            a=v(r,k(wi,ux(p),uy(q)));
            Pm(p,q)=Pm(p,q)+(a'*a)/(a'*Un*a);
        end
    end
end
