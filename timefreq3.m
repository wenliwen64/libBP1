...
% for j=1:nel
%     xorin(j,:)=1*sin(2*pi*40*tt-sd(j))+sin(2*pi*10*tt-sd(j));
% end
for j=1:nel
    xr(j,:)=coe(j)*x1(j,:);
end
...
s=exp(log(200):log(0.6)/4:log(2));
 ww=zeros(nel,length(s),L*sr);
% for i=1:sr
%     ww(i,:)=cwt(xorin(1,:),1/(1.03*i/200),'morl');
% end
for i=1:nel
    ww(i,:,:)=cwt(xorin(i,:),s,'morl');
end
wp=zeros(length(s),L*sr);
wp(:,:)=ww(1,:,:);

ff=exp(linspace(log(1),log(100),length(s)));
wp1=interp1(s,wp,wrev(s));
...
shading interp;
        ...
        w1=tfrwv([xorin(1,:)' xorin(2,:)'],1:L*sr,200);
        %ww=w1*0;
        tfRxx=zeros(length(s),sr*L,nel,nel);
        tfRxxS=zeros(length(s),sr*L,nel,nel);
        Wco=zeros(length(s),sr*L,nel,nel);
        AWco=zeros(length(s),sr*L);
for i=1:nel
    for j=1:nel
       [i        j]
        for ii=1:length(s)
            for jj=1:sr*L
                tfRxx(ii,jj,i,j)=1/s(ii)*ww(i,ii,jj)*ww(j,ii,jj)';
            end
        end
        ...
        for ii=1:length(s)
            ...
            tempcov=zeros(1,sr*L);
              for jj=1:sr*L
                  ...
                  temp(jj)= mean(tfRxx(silow:sihigh,jj,i,j));
              end
...
%                   end
%                   
%                 tfRxxS(ii,jj,i,j)= mean(temp(tilow:tihigh).*exp(-((tt(:)-tt(jj))*1e3).^2/(2*s(ii)^2)));
%             end
                    if s(ii)<=5
                        ss=5;
                    else
                    ...
                    ss=s(ii);
                    tempcov=conv(temp,exp(-((tt(1:2*round(ss/sr/2/(1/sr)))-tt(1)+0.05-s(ii)/200/2)*2e2).^2/(2*ss^2)),'same');
                    tfRxxS(ii,:,i,j)=NaN;
                    %tfRxxS(ii,:,i,j)=temp(:);
                    tfRxxS(ii,round(ss/sr/2/(1/sr)):sr*L-round(ss/sr/2/(1/sr)),i,j)=tempcov(round(ss/sr/2/(1/sr)):sr*L-round(ss/sr/2/(1/sr)));
...
for i=1:nel
    for j=1:nel
        for ii=1:length(s)
            for jj=1:sr*L
                Wco(ii,jj,i,j)=tfRxxS(ii,jj,i,j)*tfRxxS(ii,jj,i,j)'/(tfRxxS(ii,jj,i,i)*tfRxxS(ii,jj,j,j));
            end
        end
...
for i=1:nel
    for j=1:nel
        for ii=1:length(s)
            for jj=1:sr*L
                if i~=j
                AWco(ii,jj)=AWco(ii,jj)+1/(nel*(nel-1))*Wco(ii,jj,i,j);
                end
                  %AWco(ii,jj)=tfRxxS(ii,jj,1,2);
            end
        end
        
    end
end
X=zeros(length(s),sr*L);
Y=zeros(length(s),sr*L);
% XI=zeros(length(s),sr*L);
% for i=1:length(s)
%     Y(i,:)=tt;
% end
for i=1:sr*L
...
% 
% ff=1/200:1/200:1/2;
%         AWcof=zeros(length(ff),sr*L);
% for i=1:sr*L
%     
% AWcof(:,i)=interp1(s,AWco(:,i),1./ff);
% end
figure(3);
pcolor(Y,X,AWco.^0.5);
colorbar;
