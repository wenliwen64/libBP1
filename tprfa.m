...
lat=zeros(1,length(dist));
 ...
 latj=4*cosd(a);
 lonj=4*sind(a)/cosd(lat0);
for i=1:N
    
    lat(i)=lat0-dist(i)*cosd(b)/111.94;
    lon(i)=lon0-dist(i)*sind(b)/cosd(lat0)/111.94;
    ...
    lat2=linspace(lat1-0.5*cosd(b),lat1+1*cosd(b),100);
    lon2=linspace(lon1-0.5*sind(b)/cosd(lat0),lon1+1*sind(b)/cosd(lat0));
