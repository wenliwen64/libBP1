...
def=struct('win',9,...% length of the window for the cross correlation
    ...
    'range',20,...% plus and minus range of the possible time shift
 ...
 elseif shiftT1bool==true;
     ...
     for k=1:nel
         for j=1:lt
             ...
             xx=x(k,ts22*sr:(ts22+win)*sr);
             xxi2(k,j)=sqrt(sum(xx.^2));
             %                                    xcr(k,i,j)=-sum(abs(xx0-xx));
         end
     end
     
     for k=1:nel
         k
         xx0=x(k,(ts11-5*range)*sr:(ts11+win+5*range)*sr);%reference seismograms
         xx0(1:5*range*sr-1)=0;
         xx0(end-5*range*sr+1:end)=0;
         xx02=sum(xx0.^2);
...
%              ['k= ' num2str(k) ' i= ' mum2str(i)]
             ...
             xx=x(i,(ts11-5*range)*sr:(ts11+win+5*range)*sr);
             
             [xcrtmp , lags]=xcorr(xx,xx0,(lt-1)/2);
%              for j=1:lt
%                  %                  clear tip xx;
%                   ts22=ts11+tou(j);
% % %                  tip=linspace(ts22,ts22+win,win*sr+1);
%                   xx=x(i,(ts22-5*range)*sr:(ts22+win+5*range)*sr);
% %                  xcr(k,i,j)=sum(xx0.*xx)/(xxi2(i,j)*xxi2(k,(lt+1)/2));
%                                   xcr(k,i,j)=sum(xx0.*xx);
% 
%              end
             
             xcr(k,i,:)=xcrtmp./(xxi2(i,:)*xxi2(k,(lt+1)/2));
