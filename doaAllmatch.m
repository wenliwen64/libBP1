function ret=doaAll6(ret,ret1,opr)
...
for tl=ori+begin:step:ori+over % looping time
    ...
    if strcmp(method,'interf')% stacking part
        ...
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    if strcmp(method,'match')% stacking part
        Lh=length(x0);
        y=zeros(nel,Lh);
        y0=zeros(nel,windowLength*sr);
        for k=1:nel
            y0(k,:)=ret1.x(k,ret1.tb*sr:ret1.tb*sr+windowLength*sr-1);
        end
        for p=1:ps
            p
            for q=1:qs
                ...
                for k=1:nel
                    ...
                    y(k,(tl-1*windowLength)*sr:(tl+2*windowLength)*sr)=specshift(x0(k,(tl-1*windowLength)*sr:(tl+2*windowLength)*sr),sd(k)*sr);
                    y1(k,:)=y(k,tl*sr:tl*sr+windowLength*sr-1);%/std(y(k,tl*sr:tl*sr+windowLength*sr-1));
                    corryy(k)=sum(y0(k,:).*y1(k,:))/sqrt((sum(y0(k,:).^2)*sum(y1(k,:).^2)));
                end
                
                
                Pm(p,q)=mean(abs(corryy));
%                   figure(17);
%                   hold on;
%         plot(1:length(y1),y1(1,:));
%         plot(1:length(y0),y0(1,:),'r');
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
     if strcmp(method,'emusic')% Multitaper-Music back projection
        Lh=length(x0);
        y=zeros(nel,Lh);
        ...
        for i=fli:fs:fhi
            ...
            for p=1:ps
                ...
                for q=1:qs
                    ...
                    a=exp(-1i*2*pi*vi*tlib(:,p,q));%+ret.mf(kkk,:)';
%                     a=a./ret.mf(kkk,:)';
%                      Pm1(p,q)=(wi*(a'*Rxx*a)/(a'*a));
                    Pm1(p,q)=(wi*(a'*a)/(a'*Un*a));% MUSIC spectrum
                    Pw1(p,q)=(wi*(a'*Rxx*a)/(a'*a)); % beamforming power
                end
            end
    ...
    plot(lon0,lat0,'g*','MarkerSize',20);
    if isfield(ret,'lon00')
%     plot(ret.lon00,ret.lat00,'k*','MarkerSize',20);
    end
