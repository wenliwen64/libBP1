...
% %      k=k+1;
 ...
 for per=1:80
 plot([180:360],(isec(:,per)-mean(isec(:,per)))/max(isec(:,per)));
...
for theta=0:180
 ...
 plot([180:360],tot/80,'g','LineWidth',4);
 hold off;
...
%             plot(-lata(1:length(lata1)),x1/scale1+lona1(1),'b');
...
clear thek;
subplot(3,1,2)
 x1=real(hilbert(x)*exp(-1i*(themin+180)/180*pi));
plot(1:length(x1),x1);
