...
for j=1:nel
 ...
 %ts2=0.4;
  ...
  %x1(1,:)=sin(2*pi*tt+0.947).*sin(2*pi*40*tt);
...
for j=1:nel
   ...
   y(j,1:window*sr)=x(j,round((ts1+sd(j))*sr)+1:round((ts1+sd(j))*sr)+window*sr)+0*std(x1(1,:))*rand(1,window*sr);
end
%y=x;
...
for i=1:nel
    for j=1:nel
          [c ph ci ]=cmtm2(y(i,tl*sr:th*sr-1)-mean(y(i,tl*sr:th*sr-1)),y(j,tl*sr:th*sr-1)-mean(y(j,tl*sr:th*sr-1)),5);
          for k=1:length(s)
              S(k,i,j)=c(k);
          end
    end
...
%      for k=1:20
%         for j=1:nel
%             X(j,:)=fft(y(j,tl*sr+(k-1)*2*sr:tl*sr+(k)*2*sr));
%         end
%          Rxx=Rxx+X(:,i)*X(:,i)';
%     end
